    ...
    <div ref={setNodeRef}
      ...
      <div className="flex items-center justify-between border-b border-gray-200/60 px-4 py-3 dark:border-gray-700/40">
        <div className="flex items-center gap-2">
          <div className="rounded-md bg-gray-200/60 p-1.5 dark:bg-gray-700">
            <col.icon className={`h-4 w-4 ${col.color}`} />
          </div>
          <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300">{STATUS_LABELS[status]}</h3>
        </div>
        <span className="flex h-5 min-w-[20px] items-center justify-center rounded-full bg-gray-200/80 px-1.5 text-[11px] font-semibold text-gray-600 dark:bg-gray-700 dark:text-gray-400">{count}</span>
      </div>
      <div className={`flex flex-col gap-2 p-3 ${isOver ? 'bg-primary/5' : ''}`}>
        {children}
      </div>
      <div className="mx-3 mb-3 flex flex-1 items-center justify-center rounded-lg border-2 border-dashed border-gray-200 p-5 transition-colors hover:border-gray-300 dark:border-gray-700 dark:hover:border-gray-600">
        {adding ? (
